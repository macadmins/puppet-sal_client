  ...
  class << self
    ...
    def sorted_generate(obj)
      ...
      when String
        ...
        return (obj =~ /\A[-]?[0-9]+\z/ ? obj.to_i : obj).to_json
      when Array
        array_ret = []
        obj.each do |a|
    ...
    def sorted_pretty_generate(obj, indent_len = 4)
      ...
      when Hash
        ...
        @@loop += 1
        obj.keys.sort.each do |k|
          ret.push((indent * @@loop).to_s << k.to_json << ': ' << sorted_pretty_generate(obj[k], indent_len))
        end
        @@loop -= 1
...
module Puppet::Parser::Functions
  ...
  *Examples:*
    ...
    unsorted_hash.reject! { |_key, value| value == :undef }
